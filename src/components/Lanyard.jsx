...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false, frontImage = null, backImage = null, imageFit = 'cover', lanyardImage = null, lanyardWidth = 1 }) {
  ...
  const cardMap = useMemo(() => {
    ...
    // draw the original base (back half stays intact)
    ...
    // front face: wipe the base design clean, fill solid dark bg, then draw logo centered
    if (frontImage && frontTex.image) {
      const rx = FRONT_UV_RECT.x * W, ry = FRONT_UV_RECT.y * H;
      const rw = FRONT_UV_RECT.w * W, rh = FRONT_UV_RECT.h * H;
